...
class ProjectionMaker:
    ...
    def __init__(self,h5,vidx=0,volume_name='/processed_data',avol=None):
        self.logger = logging.getLogger(__name__)

        if type(h5)==str:
            h5 = H5(h5)
        self.logger.info('Extracting volume from hdf5 file.')
        self.avol = np.abs(h5.get(volume_name))[vidx,:,:,:]
...
class VolumeProjectionMaker:
    ...
    def project(self):
        ...
        return self.areal_projection,min(clicks[-2],clicks[-1]),max(clicks[-2],clicks[-1])
